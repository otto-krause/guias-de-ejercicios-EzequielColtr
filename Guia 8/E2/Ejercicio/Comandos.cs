...
namespace Ejercicio
{
    public class Comandos
    {
        ...
        {
            using (StreamReader r = new StreamReader("locales-original.json"))
            {
                json = r.ReadToEnd();
            }
        ...
        public bool BuscarTrabajo(double longitud, double latitud){
            ...
            bool res= false;
            List<Local> localesCercanos = root.locales.OrderBy(local => local.coordenadas.latitud + local.coordenadas.longitud - longitud + latitud).ToList();
